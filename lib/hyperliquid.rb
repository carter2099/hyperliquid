...
module Hyperliquid
  ...
  def self.new(testnet: false, timeout: Constants::DEFAULT_TIMEOUT, retry_enabled: false, private_key: nil)
    SDK.new(testnet: testnet, timeout: timeout, retry_enabled: retry_enabled, private_key: private_key)
  end
  ...
  class SDK
    ...
    # @param retry_enabled [Boolean] Whether to enable retry logic (default: false)
    # @param private_key [String, nil] Ethereum private key for exchange operations (optional)
    def initialize(testnet: false, timeout: Constants::DEFAULT_TIMEOUT, retry_enabled: false, private_key: nil)
      ...
      signer = Signing::Signer.new(private_key: private_key, mainnet: !testnet)
      @exchange = Exchange.new(client: client, signer: signer, info: @info)
    end
